...
class GatedConvLayer(nn.Module):
    ...
    def __init__(self, in_channels, nfeats, kernel_size=3, mask_type="A"):
        ...
        self.vconv = MaskedConv(
            in_channels=in_channels,
            out_channels=2 * nfeats,
            kernel_size=kernel_size,
            ver_or_hor="V",
            mask_type=mask_type,
        )

        self.hconv = MaskedConv(
            in_channels=in_channels,
            out_channels=2 * nfeats,
            kernel_size=kernel_size,
            ver_or_hor="H",
        ...
        self.v_to_h_conv = nn.Conv2d(
            in_channels=2 * nfeats, out_channels=2 * nfeats, kernel_size=1
        )  # 1x1 conv

        self.h_to_h_conv = nn.Conv2d(
            in_channels=nfeats, out_channels=nfeats, kernel_size=1
        )  # 1x1 conv
...
class PixelCNN(nn.Module):
    ...
    def __init__(self, nlayers, in_channels, nfeats, Klevels=2, ksz_A=5, ksz_B=3):
        ...
        for i in range(nlayers):
            gatedconv = GatedConvLayer(
                in_channels=nfeats, nfeats=nfeats, mask_type="B", kernel_size=ksz_B
            )
            self.layers.append(gatedconv)
        # TODO make kernel sizes as params

        self.out_conv = nn.Sequential(
            nn.Conv2d(nfeats, nfeats, 1), nn.ReLU(True), nn.Conv2d(nfeats, Klevels, 1)
        )
